...
def do_arp_ping(ip_address: IPv4Address) -> list[tuple[str, str]]:
    ...
    ETHER_FAILED = "__:__:__:__:__:__" # I really hate strings
    arp_request = Ether(dst = ETHER_BROADCAST) / ARP(psrc = SELF_IP.exploded, pdst = ip_address.exploded) # I. hate. strings.
    ...
    return [(reply.answer.hwsrc, reply.answer.psrc) for reply in answers] + [(ETHER_FAILED, pkt.pdst) for pkt in noreplies]
